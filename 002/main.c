...
//2019/03/02 IBOTIAndy A.M. 00:40
...
void matrixMath(int type, int row, int col, int n, int matrix1[][10], int matrix2[][10], int matrixans[][10]){
    int y=0, x=0;
    for(y=0; y < row; y++){
        for(x=0; x < col; x++){
            if(type == 0){
                ...
                matrixans[y][x] = matrixMultiply(y, x, n, matrix1, matrix2);
            }
        }
    }
}
...
void run(int row1, int col1, int matrix1[][10], int row2, int col2, int matrix2[][10]){  //
    ...
    if(row1 == row2 && col1 == col2){
        matrixMath(0, row1, col1, n, matrix1, matrix2, addans);
        matrixMath(1, row1, col1, n, matrix1, matrix2, subans);
    ...
    if(col1 == row2){
        n = col1;   //or "n = row2" 紀錄要重複幾次乘法，因為例如2x3 3x2 結果會是2x2但是計算時會3次乘法相加
        matrixMath(2, row1, col2, n, matrix1, matrix2, mulans);
        output(row1, col2, mulans);
    }
    else{
...
//    matrix2[3][0] = 7; matrix2[3][1] = 4; matrix2[3][2] = 2;
    ...
    *col2 = 3;
    ...
    matrix1[1][0] = 2; matrix1[1][1] = 0;
    matrix2[0][0] = 5; matrix2[0][1] = 0; matrix2[0][2] = 1;
    matrix2[1][0] = 3; matrix2[1][1] = -2; matrix2[1][2] = 6;
